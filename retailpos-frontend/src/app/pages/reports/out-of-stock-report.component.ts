...
@Component({
  ...
  styleUrls: ['./out-of-stock-report.component.css']
})
export class OutOfStockReportComponent implements OnInit, OnDestroy {
  // ── Filters ───────────────────────────────────────────────────────────────
  ...
  ngOnDestroy(): void {
    this.reportService.outOfStockReport.set(null);
  }

  // ── Computed ──────────────────────────────────────────────────────────────
  report     = computed(() => this.reportService.outOfStockReport());
  ...
  loadReport(): void {
    ...
    this.reportService.getOutOfStockReport({
  ...
  export(format: 'csv' | 'excel'): void {
    const url = this.reportService.buildOutOfStockExportUrl({
      outletId:    this.outletId()    ?? undefined,
      warehouseId: this.warehouseId() ?? undefined,
      categoryId:  this.categoryId()  ?? undefined,
      search:      this.search()      || undefined,
